...
class RainbowCanvas extends React.Component {
	...
	handleWidth(e) {
		...
		if((ctx.lineWidth >= this.state.maxWidth && this.state.direction === true)
	...
	handleInputChange(event) {
		...
		} else if (name === "customeStroke" && value === false) {
			this.ctx().lineWidth = Number(this.state.minWidth)+1;
		}
	}
	...
	render() {
		...
		return (
			<div>
				...
				<Controls left={this.state.controlLeft}
...
function StrokeCheckbox(props) {
	return (
		<div>
			<label>  Fixed Stroke width </label>
			<input name="customStrike" type="checkbox" onChange={props.handleChange} value={props.checked} />
		</div>
	)
}
